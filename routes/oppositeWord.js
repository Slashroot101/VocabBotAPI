...
var oppositeWord = require('../DataModels/oppositeWord');
...
router.use(function(req, res, next){
    ...
    User.findOne({
        ...
        if(data.availablePoints >= config.weights.oppositeWordWeight || data.admin){
            next();
        } else {
...
//saves a oppositeWord if one doesn't already exist matching it, if it does, it skips saving it, and does not reward any points
router.post('/create', function (req, res, next) {
    console.log(`Posting string word!`);
    var newWord1 = new oppositeWord({
    ...
    oppositeWord.findOne(
        {
            ...
            if (data) {
                var token = req.body.token || req.query.token || req.headers['x-access-token'] || req.cookies.token;
                var decoded = jwt.decode(token);
                User.updateOne({ name: req.query.addedBy }, { $inc: { availablePoints: config.weights.oppositeWordWeight * -1 } }, function (err, dataUser) {
                    console.log(err);
                    if (err) throw err;
